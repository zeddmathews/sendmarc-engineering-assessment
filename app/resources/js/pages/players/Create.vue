...
import type { User, PlayerPageProps } from '@/types';
...
const form = useForm({
    ...
    user_id: isAdmin ? '' : authUser.id,
});

const submit = () => form.post(route('api.players.store'));
</script>
    ...
    <AppLayout>
        ...
        <div class="max-w-lg mx-auto p-6 bg-gray-900 shadow rounded">
            ...
            <form @submit.prevent="submit" class="space-y-4">
                ...
                <button type="submit" :disabled="form.processing" class="px-4 py-2 bg-green-500 text-gray-900 rounded hover:bg-green-600 disabled:opacity-50">Create Player</button>
            </form>
        </div>
    </AppLayout>
</template>
