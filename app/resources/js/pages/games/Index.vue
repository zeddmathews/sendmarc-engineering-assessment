...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <div v-if="games.length" class="grid grid-cols-3 gap-6">
            <div
            v-for="game in games"
            :key="game.id"
            class="border p-4 rounded shadow hover:shadow-lg cursor-pointer relative"
            >
            ...
            <div v-if="isAdmin" class="mt-4 flex gap-2 justify-end">
                <Link
                :href="route('games.edit', game.id)"
                class="inline-block px-3 py-1 text-sm bg-indigo-600 hover:bg-indigo-700 text-white rounded"
                >
                Edit
                </Link>
                <Button
        ...
        <div v-else class="text-center text-gray-500 mt-6">No games found.</div>
        </div>
    </AppLayout>
</template>
